...
Delivery posture: implement only local CLI prep plus stage-1 visual extraction and Stage 2 structural reconstruction against explicit PDF and page-range inputs, retain inspectable prep, OCR, OCR-anchor, mark-observable, logical-page, and block-unit evidence, and leave stages 3-4 plus the two markdown terminal artifacts as future work under the same proof contract.
...
- Task constraints: keep claims at prep plus stage-1 visual extraction and Stage 2 structural reconstruction from retained Stage 1 OCR and mark-observable evidence; treat PDF as the input format; make PDF-to-PNG conversion explicit; keep controls generic for the scanned-novel object class; do not parse repo config in runtime code; do not claim stages 3-4 or final artifacts exist.
- Constraint conflicts: the active proof contract still names later stages and terminal artifacts as required future behavior, while the live implementation stops at Stage 2; later seams must preserve that distinction. The Stage 1 prerequisite for Stage 3 now exists as retained OCR anchor geometry, visual mark masks, mark candidate tables, and geometric candidate-to-anchor relations, but Stage 3 interpretation itself remains unimplemented. Older Stage 1 and Stage 2 validation roots created before spread rectification remain mechanically useful as evidence of prior behavior, but they are stale for intent-level claims about upright whole-page OCR on rotated spreads.
...
- Stage 1 now also retains per-surface OCR anchor tables, visual mark masks, mark candidate tables, and geometric candidate-to-anchor relations. Those observables remain visual-only: they do not resolve final text spans, interpret marginalia meaning, or synthesize artifact entries.
- The stage-1 manifest records the selected Tesseract path, selection rule, candidate probes, selected pages, declared scan layout, resolved spread handling, requested and applied spread rotation, per-surface evidence paths, OCR confidence summaries, OCR anchor counts, and annotation-candidate summaries.
...
- Passed: fresh default-path full-packet run on PDF pages 20-28 with omitted `--spread-rotation-deg`, recording `applied_spread_rotation_deg` 90 with `spread_rotation_defaulted` true, retaining 9 raw spread PNGs plus 9 rectified spread PNGs, and emitting 18 upright whole-page derived surface PNGs plus 18 OCR text files.
- Passed: default-path full-packet Stage 2 rerun on that root, producing 18 logical pages for `page-units`, then 18 logical pages plus 146 `block-unit-*.txt` files for `block-units`, matching the corrected explicit-90 full-packet counts.
...
- Passed: focused corrected-root Stage 1 annotation probe on PDF pages 20 and 28 at `C:\Users\madis\AppData\Local\Temp\novel-stage1-annotation-probe-v4-1ac275c9-5665-4e1a-95d0-2242eb04bc71\anchor-probe-v4`, where the marginalia anchor surface `pdf-page-0020-right` retained 20 candidates with 2 confident highlight-or-underline candidates and marginalia candidates carrying either `marginal-adjacent` relations or explicit `no-ocr-anchor` uncertainty, while clean surfaces `pdf-page-0028-left` and `pdf-page-0028-right` retained no confident non-unknown candidates.
- Passed: corrected-root Stage 1 anchor-suite validation on PDF pages 20, 24, 25, and 28 at `C:\Users\madis\AppData\Local\Temp\novel-stage1-anchor-suite-v2-bd4c44ec-4ded-4efd-b06f-9c8c4154a2ce\anchor-suite-v2`, where `pdf-page-0020-right` retained marginalia candidates with anchor-or-uncertainty support, `pdf-page-0024-left` retained ambiguous anchor geometry without forcing a single line choice, `pdf-page-0025-right` retained only uncertain non-unknown candidates after the dense-margin-dark-noise downgrade, and clean surfaces `pdf-page-0028-left` and `pdf-page-0028-right` still retained no confident non-unknown candidates.
- Passed: corrected-root Stage 1 evidence-integrity probe on the same anchor-suite root found zero missing manifest-referenced evidence paths and zero unresolved `anchor_relations.anchor_id` references, and the explicit ambiguity assertion on `pdf-page-0024-left` confirmed `ambiguous_candidate_present = true` with `confident_bracket_or_box_present = false`.
...
- The repo root now contains a runnable `python -m novel_and_hilite_extraction stage1-visual-extract` surface for prep plus stage-1 OCR, OCR-anchor, and mark-observable evidence.
...
- The live seam now also retains OCR anchor geometry, mark masks, mark candidate tables, and geometric candidate-to-anchor relations on corrected roots, with clean and bleed-through anchors staying uncertain instead of being promoted as confident annotation truth.
