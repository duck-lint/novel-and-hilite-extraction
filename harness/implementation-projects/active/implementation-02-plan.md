...
1. Corrected-root unresolved inventory and deterministic case-family definition.
   Enumerate the current 117 unresolved corrected-root interpretations, group them into deterministic semantic families grounded in current observables, and document what evidence distinguishes each family.

2. Bounded broader Stage 3 semantics for non-promoted families.
   Broaden Stage 3 semantics only for families whose behavior can be made more informative without forcing confident text-span interpretation. Prefer richer unresolved semantics or deterministic classification over promotion.

3. Corrected-root acceptance rerun and regression guard.
   Re-run the corrected-root proof probe to show no regression on the current five clean resolutions and no over-promotion on the known ambiguity, bleed-through, marginalia, or clean-surface anchors.
...
- Implementation-01 remains archived while implementation-02 is the only live bundle under `active/`.
- A corrected-root probe shows broader Stage 3 semantic differentiation than the current clean highlight-or-underline-only boundary while preserving explicit unresolved carry-forward.
- The existing five corrected-root clean highlight-or-underline resolutions remain present or improve without regression.
...
- Stage 3 currently resolves only clean highlight or underline candidates, preserves anchor-local `resolved_span_text`, exposes widened `semantic_entry_text`, and now differentiates unresolved cases into deterministic families such as `missing-anchor-and-empty-text-unresolved`, `ambiguous-underline-anchor-unresolved`, `ambiguous-highlight-anchor-unresolved`, `ambiguous-anchor-unknown-mark-unresolved`, `marginalia-unresolved`, `low-confidence-unresolved`, `enclosure-like-unknown-mark-unresolved`, `enclosure-like-underline-unresolved`, and `stage1-uncertainty-unresolved`, with `unresolved-anchor-reference-unresolved` and `enclosure-like-highlight-unresolved` available but unpopulated on the current corrected-root proof surface.
...
- A second fresh corrected-root rerun at `C:\Users\madis\AppData\Local\Temp\novel-stage3-no-anchor-split-fbdfdacf-ab59-447e-95fc-0d46e6c09d16\anchor-suite-stage1` preserved the same 122 total interpretations with 5 resolved and 117 unresolved while replacing the prior 36-entry `no-anchor-evidence-unresolved` bucket with a more specific 36-entry `missing-anchor-and-empty-text-unresolved` bucket and showing zero `unresolved-anchor-reference-unresolved` cases on the current proof surface.
- A third fresh corrected-root rerun at `C:\Users\madis\AppData\Local\Temp\novel-stage3-ambiguous-split-a496396a-e3a2-43c1-9cbb-116a02af7368\anchor-suite-stage1` preserved the same 122 total interpretations with 5 resolved and 117 unresolved while replacing the prior 32-entry `ambiguous-anchor-unresolved` bucket with 27 `ambiguous-underline-anchor-unresolved`, 3 `ambiguous-highlight-anchor-unresolved`, and 2 `ambiguous-anchor-unknown-mark-unresolved` entries, again without promoting additional candidates or resolving any bracket-or-box interpretation.
...
- Partially resolved: the current manifest shape also supports a narrower no-anchor split without changing the payload contract; on the present corrected-root proof surface, that split reveals a populated combined missing-anchor-plus-empty-text family and no unresolved-anchor-reference cases.
- Partially resolved: the current manifest shape also supports a narrower ambiguous-anchor split without changing the payload contract; on the present corrected-root proof surface, that split reveals mostly underline-shaped ambiguity, a smaller highlight-shaped ambiguity slice, and a very small unknown-mark ambiguity slice.
- Partially resolved: the current manifest shape also supports a narrower enclosure-like split without changing the payload contract; on the present corrected-root proof surface, that split reveals mostly unknown-mark enclosure cases plus a single underline-shaped enclosure case.
...
- Unknown: whether the next governing semantic tension is now marginalia, low-confidence, or a deeper ambiguity contract beyond the present mark-form split.
