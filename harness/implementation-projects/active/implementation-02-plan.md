...
- Stage 3 currently resolves only clean highlight or underline candidates, preserves anchor-local `resolved_span_text`, exposes widened `semantic_entry_text`, and now differentiates unresolved cases into deterministic families such as `missing-anchor-and-empty-text-unresolved`, `ambiguous-underline-anchor-unresolved`, `ambiguous-highlight-anchor-unresolved`, `ambiguous-anchor-unknown-mark-unresolved`, `marginalia-unresolved`, `low-confidence-unresolved`, `enclosure-like-unresolved`, and `stage1-uncertainty-unresolved`, with `unresolved-anchor-reference-unresolved` available but unpopulated on the current corrected-root proof surface.
...
- Unknown: whether the next governing semantic tension is now deeper ambiguity semantics over the remaining ambiguous underline and highlight families, `enclosure-like-unresolved`, or another still-unresolved family such as marginalia or low-confidence.
