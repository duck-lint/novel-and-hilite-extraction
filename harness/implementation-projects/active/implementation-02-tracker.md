...
- Next action: reassess whether the next governing Stage 3 seam is now `ambiguous-anchor-unresolved` or `enclosure-like-unresolved`, because the no-anchor split is complete and the current corrected-root proof surface shows a single populated `missing-anchor-and-empty-text-unresolved` family with zero unresolved-reference cases
...
| Seam 2: bounded broader Stage 3 semantics for non-promoted families | Implementer | active | `stage3.py` now assigns deterministic unresolved `interpretation_kind` values from existing reason codes, including a completed no-anchor subfamily split, while preserving the five clean resolutions and the 117 unresolved carry-forward population on fresh corrected-root reruns | Stop if deeper semantics require a contract-change approval |
